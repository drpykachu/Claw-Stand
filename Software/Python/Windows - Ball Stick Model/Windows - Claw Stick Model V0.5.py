...
def solve_thetas(Zp, Yp, Xp, A, B, C, T, Offset_R, Offset_theta):
    ...
    Xp = Xp - Offset_R*np.cos(np.deg2rad(Offset_theta))
    Yp = Yp - Offset_R*np.sin(np.deg2rad(Offset_theta))
...
def plotter(theta_a,theta_b,theta_c, Offset_R, Offset_theta):
    ...
    x0 = 0 
    y0 = 0     
    x0 = 0 + Offset_R*np.cos(np.deg2rad(Offset_theta))
    y0 = 0 + Offset_R*np.sin(np.deg2rad(Offset_theta))
    z0 = 0 
...
num_fingers = 3   # Number of fingers  
...
def update(val):
    ...
    ax.plot([0, 0], [0, 0], [-lims/10, lims/10], '--', c = 'k', lw = 2,alpha = 0.2)

    Xtar_i, Ytar_i, Ztar_i  = circle_tar[0][int(val)], circle_tar[1][int(val)], circle_tar[2]
    
    for k in range(num_fingers):

        Offset_theta = Offset_theta_master[k]
        
        Xtar, Ytar = rotate((0,0), (Xtar_i, Ytar_i),Offset_theta)
        Ztar = Ztar_i
#         print('Postrotate:  %.2f  %.2f' % (Xtar,Ytar))
#         print()
...
#             print('No physical solution found for %.2f , %.2f, %.2f.' % (Xtar, Ytar, Ztar), end='\r', flush=True)
        ...
        ax.plot([Xtar], [Ytar], [Ztar], '.b', markersize=8,zorder = 100)
        ax.text(Xtar+5, Ytar+5, Ztar, str(k+1), color='c')
...
slider = tk.Scale(root, from_=-90, to=90, orient="horizontal", length=400,
