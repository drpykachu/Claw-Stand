...
for MOTOR_ID in MOTOR_ID_LIST:
    set_speed(500, MOTOR_ID) # 100–600 recommended
...
H_tar       = 140  # Height of target circle path
...
# Initialize current angles for all joints & fingers
current_theta = np.full((3, num_fingers), np.nan)  # nan = uninitialized

# Precompute spacing/order for star pattern
step = 2  # star step for odd fingers
spacing = int(points / num_fingers)
...
while True:
    ...
    for shift_idx, finger_idx in enumerate(order):
        try:
            # Target coordinates
            Xtar, Ytar, Ztar = master_path[finger_idx, :, val]
            Offset_theta = Offset_theta_master[finger_idx]

            # Rotate target point by finger offset
            Xtar_new, Ytar_new = rotate_point((0, 0), (Xtar, Ytar), Offset_theta)

            # Solve inverse kinematics
            theta_target = solve_thetas(Ztar, Ytar, Xtar, A, B, C, T, Offset_R)[0]

            # Initialize current angles if first loop
            if np.isnan(current_theta[0, finger_idx]):
                current_theta[:, finger_idx] = theta_target

            # Gradually step each joint
            for joint in range(3):
                err_deg = np.rad2deg(theta_target[joint] - current_theta[joint, finger_idx])
                if abs(err_deg) > minstep:
                    step_val = np.sign(err_deg) * np.deg2rad(minstep)
                    current_theta[joint, finger_idx] += step_val
                else:
                    current_theta[joint, finger_idx] = theta_target[joint]

            # ==================== SEND TO MOTORS ====================
            # Map your joints to motor IDs (example for first finger)
            if finger_idx == 0:
                theta_a, theta_b, theta_c = current_theta[:, finger_idx]
            ...
            print(f'IK Error for finger {finger_idx}: {e}', end='\r')
